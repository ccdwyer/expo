...
For JS Tabs, `href: null` hides the tab, makes its route unreachable through that navigator, and redirects navigation attempts to the initial visible tab.
...
```ts
import { extendRouter, StackRouter } from 'expo-router';
...
Import the base and native stack helpers from `expo-router`. Import the navigator-specific helpers from the corresponding `expo-router/js-stack`, `expo-router/js-tabs`, `expo-router/js-top-tabs`, or `expo-router/unstable-native-tabs` entry point.
...
> **warning** The custom navigator guide's options example applies to SDK 57. Omit `useOnlyUserDefinedScreens` and declare props returned by `createProps` in the fourth `NavigatorContentProps` type argument when using it with SDK 58.
