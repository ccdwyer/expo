...
Because the value persists, removing the option from your `configure` call does not take effect immediately. Recording returns to the default when `configure` next runs, so requests made before that point on that launch still follow the value you set previously. The same applies to a narrow `hosts` list: it keeps suppressing early-startup requests on every launch until a `configure` widens it.
